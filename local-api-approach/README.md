# Local API Approach - Browser Control API

## Overview

A lightweight FastAPI server that exposes a local browser control endpoint to Microsoft Foundry voice agents. This approach enables cloud-based voice agents to control your local browser through simple HTTP requests.

## Architecture

```
┌─────────────────┐     Voice Input      ┌──────────────────────┐
│   User/Phone    │ ───────────────────> │  Foundry Voice Agent │
│                 │                       │   (Cloud-hosted)     │
└─────────────────┘                       └──────────────────────┘
                                                     │
                                                     │ HTTP GET /api/open-browser?url=...
                                                     ↓
                                          ┌──────────────────────┐
                                          │   Dev Tunnel (HTTPS) │
                                          │   Public Endpoint    │
...
1. **User speaks** to a Microsoft Foundry voice agent (e.g., via phone or Teams)
2. **Agent processes** the voice command (e.g., "Open google.com")
3. **Agent calls** the local API via dev tunnel: `GET /api/open-browser?url=https://google.com`
4. **Local server** receives the request and opens the URL in the default browser
5. **Response** is sent back to the agent confirming the action

## Key Features

- **Simple REST API**: Single endpoint design for easy integration
- **Cloud-to-Local Bridge**: Works with cloud-based Foundry voice agents
- **OpenAPI Specification**: Includes `openapi.json` for Foundry tool configuration
- **Minimal Dependencies**: Only requires FastAPI and Uvicorn
- **Health Monitoring**: Built-in health check endpoint
- **Error Handling**: Graceful handling of headless systems
...
**Important**: Configure your Foundry voice agent with the system prompt from [AGENT_SYSTEM_PROMPT.md](AGENT_SYSTEM_PROMPT.md). This ensures the agent knows when and how to use the browser control tool.
